document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateVariableInputs() {
        ...
        const varNames = Array.from({ length: 26 }, (_, i) => String.fromCharCode(65 + i)); // A, B, C, ..., Z
    ...
    function updateFormula() {
        const numVars = parseInt(numVarsInput.value);
        const varNames = Array.from({ length: 26 }, (_, i) => String.fromCharCode(65 + i)); // A, B, C...
        let formula = '';
    ...
    btnDelete.addEventListener('click', function() {
        ...
        if (constraintRows.length > 1) {
            constraintsContainer.removeChild(constraintRows[constraintRows.length - 1]);
        } else {
    ...
    btnSolve.addEventListener('click', function() {
        ...
        let problemSummary = `${optimType === 'max' ? 'Tối đa hóa' : 'Tối thiểu hóa'} hàm mục tiêu: ${objectiveFunction}\n\n`;
        problemSummary += 'Với các ràng buộc:\n';
        
        constraints.forEach((constraint, index) => {
            problemSummary += `${index + 1}. ${constraint.left} ${constraint.operator} ${constraint.right}\n`;
        });
        ...
        addBotMessage(`Tôi đang giải bài toán tối ưu:\n${problemSummary}\nVui lòng đợi một chút...`);
        
        setTimeout(() => {
            const solution = solveProblem(objectiveFunction, optimType, constraints);
            resultArea.textContent = problemSummary + '\n\n' + solution;
    ...
    function solveProblem(objective, optimType, constraints) {
        ...
        variables.forEach(variable => {
            ...
            const favorable = (isMax && coefficients[variable] > 0) || (!isMax && coefficients[variable] < 0);
            const baseValue = favorable ? Math.floor(Math.random() * 5) + 5 : Math.floor(Math.random() * 4) + 1;
            solution[variable] = baseValue;
            totalValue += coefficients[variable] * baseValue;
        });
        
        let solutionText = `Giá trị ${optimType === 'max' ? 'tối đa' : 'tối thiểu'}: ${totalValue.toFixed(2)}\n\n`;
        solutionText += 'Giá trị tối ưu cho các biến:\n';
        
        Object.keys(solution).sort().forEach(variable => {
    ...
    function respondToMessage(message) {
        ...
        if (message.includes('xin chào') || message.includes('chào') || message.includes('hello') || message.includes('hi')) {
            response = 'Xin chào! Tôi là trợ lý giải bài toán tối ưu tuyến tính. Bạn cần giúp đỡ gì không?';
        }
        else if (message.includes('cảm ơn')) {
            response = 'Rất vui khi được giúp đỡ bạn!';
        }
        else if (message.includes('giải') || message.includes('solve')) {
            response = 'Để giải bài toán tối ưu, bạn cần:\n1. Nhập số biến và hệ số của hàm mục tiêu\n2. Chọn loại bài toán (tối đa hóa/tối thiểu hóa)\n3. Thêm các ràng buộc\n4. Nhấn nút "Giải bài toán"';
        }
        else if (message.includes('max') || message.includes('tối đa')) {
            response = 'Bài toán tối đa hóa là tìm giá trị lớn nhất có thể của hàm mục tiêu trong miền các ràng buộc. Ví dụ: Max Z = 3x + 4y với các ràng buộc x + y ≤ 10, x ≥ 0, y ≥ 0.';
        }
        else if (message.includes('min') || message.includes('tối thiểu')) {
            response = 'Bài toán tối thiểu hóa là tìm giá trị nhỏ nhất có thể của hàm mục tiêu trong miền các ràng buộc. Ví dụ: Min Z = 5x + 2y với các ràng buộc 2x + y ≥ 8, x ≥ 0, y ≥ 0.';
        }
        else if (message.includes('ràng buộc') || message.includes('constraint')) {
            response = 'Ràng buộc là các bất phương trình hoặc phương trình giới hạn giá trị của các biến. Ví dụ: 2A + 3B ≤ 15 là một ràng buộc. Bạn có thể thêm các ràng buộc bằng cách nhấn nút "Thêm ràng buộc".';
        }
        else if (message.includes('simplex') || message.includes('thuật toán')) {
            response = 'Thuật toán Simplex là phương pháp phổ biến để giải bài toán quy hoạch tuyến tính. Nó hoạt động bằng cách di chuyển từ đỉnh này sang đỉnh khác của đa diện ràng buộc, tìm kiếm giá trị tối ưu của hàm mục tiêu.';
        }
        else if (message.includes('ví dụ') || message.includes('example')) {
            response = 'Ví dụ về bài toán tối ưu tuyến tính:\n\nTối đa hóa: 3A + 2B + 5C\nRàng buộc:\n1. A + B + C ≤ 100\n2. 2A + B ≤ 150\n3. A, B, C ≥ 0';
        }
        else if (message.includes('hướng dẫn') || message.includes('guide') || message.includes('help')) {
            ...
            response = 'Biến là các giá trị cần tìm trong bài toán tối ưu. Trong ứng dụng này, bạn có thể sử dụng từ 1 đến 10 biến, được đặt tên từ A đến J.';
        }
        else if (message.includes('hàm mục tiêu') || message.includes('objective function')) {
            response = 'Hàm mục tiêu là hàm cần được tối đa hóa hoặc tối thiểu hóa trong bài toán tối ưu. Ví dụ: Z = 5A + 3B - 2C là một hàm mục tiêu.';
        }
        else {
            response = 'Tôi không hiểu câu hỏi của bạn. Bạn có thể hỏi về cách giải bài toán tối ưu, ý nghĩa của các thuật ngữ như ràng buộc, hàm mục tiêu, hoặc yêu cầu một ví dụ.';
        }
